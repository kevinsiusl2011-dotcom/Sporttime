...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: copy.privacyTitle, description: copy.privacyIntro };
}
...
export default async function PrivacyPage() {
  ...
  return (
    <AppFrame t={t} locale={locale} narrow>
      <h1 className="font-[family-name:var(--font-serif)] text-4xl">{copy.privacyTitle}</h1>
      <p className="mt-2 text-sm text-[var(--muted)]">{copy.updated}</p>
      <p className="mt-6 max-w-2xl leading-relaxed text-[var(--muted)]">{copy.privacyIntro}</p>
