...
export function NavLinks({ items, variant }: { items: NavItem[]; variant: "desktop" | "mobile" }) {
  ...
  if (variant === "mobile") {
    return (
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-[var(--line)] bg-[color-mix(in_oklab,var(--bg)_94%,white)] px-3 pt-2.5 pb-[max(0.65rem,env(safe-area-inset-bottom,0px))] backdrop-blur-xl md:hidden">
